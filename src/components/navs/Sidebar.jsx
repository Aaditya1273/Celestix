...
const Sidebar = () => {
  ...
  const findActiveElement = useCallback(() => {
    const activePath = pendingActivePath || pathname;
  ...
  }, [isTransitioning, pathname, startTransition, router]);
  ...
  useEffect(() => {
    if (pendingActivePath && pathname === pendingActivePath) {
      setPendingActivePath(null);
    }
  }, [pathname, pendingActivePath]);
...
const Category = memo(
  ({
    ...
    const items = useMemo(
      ...
      [category.name, category.subcategories, pathname, pendingActivePath]
    );
